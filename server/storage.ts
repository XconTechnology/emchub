...
export interface IStorage {
  ...
  // Messaging operations (B2C vendor-customer)
  createConversation(data: any): Promise<any>;
  getConversation(id: string): Promise<any>;
  getUserConversations(userId: string): Promise<any[]>;
  getOrCreateConversation(vendorId: string, customerId: string, relatedType?: string, relatedId?: string): Promise<any>;
  archiveConversation(conversationId: string, userId: string, archive: boolean): Promise<any>;
  ...
  // Support ticket operations (C2Admin)
  ...
  assignSupportTicket(ticketId: string, adminId: string): Promise<any>;
  
  createTicketMessage(data: any): Promise<any>;
...
export class DatabaseStorage implements IStorage {
  ...
  // ========================================
  // MESSAGING OPERATIONS (B2C)
  // ========================================
  
  async createConversation(data: InsertConversation & { vendorId: string; customerId: string }): Promise<Conversation> {
  ...
  async getUserConversations(userId: string): Promise<any[]> {
    // Get conversations where user is either vendor or customer, with latest message info
    const userConversations = await db
      .select({
        id: conversations.id,
        vendorId: conversations.vendorId,
        customerId: conversations.customerId,
        relatedType: conversations.relatedType,
        relatedId: conversations.relatedId,
        subject: conversations.subject,
        lastMessageAt: conversations.lastMessageAt,
        isArchivedByVendor: conversations.isArchivedByVendor,
        isArchivedByCustomer: conversations.isArchivedByCustomer,
    ...
    const [existing] = await db
      ...
      .where(
        and(
          ...
          relatedType ? eq(conversations.relatedType, relatedType) : sql`1=1`,
          relatedId ? eq(conversations.relatedId, relatedId) : sql`1=1`
        )
      );
  ...
  async archiveConversation(conversationId: string, userId: string, archive: boolean): Promise<Conversation> {
    ...
    const isVendor = conversation.vendorId === userId;
    const updateData = isVendor
      ? { isArchivedByVendor: archive }
      : { isArchivedByCustomer: archive };
  ...
  async createMessage(data: InsertMessage & { conversationId: string; senderId: string; receiverId: string; content: string }): Promise<Message> {
  ...
  async createSupportTicket(data: InsertSupportTicket & { userId: string; subject: string; description: string }): Promise<SupportTicket> {
  ...
  async getAllSupportTickets(filters?: { status?: string; priority?: string }): Promise<any[]> {
    ...
    return query.orderBy(sql`${supportTickets.createdAt} DESC`);
  }
  ...
  async assignSupportTicket(ticketId: string, adminId: string): Promise<SupportTicket> {
    const [ticket] = await db
      .update(supportTickets)
      .set({
  ...
  async createTicketMessage(data: InsertTicketMessage & { ticketId: string; senderId: string; content: string }): Promise<TicketMessage> {
    const [message] = await db
