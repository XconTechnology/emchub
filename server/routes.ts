...
export function registerRoutes(app: Express): Server {
  ...
  // Get ticket by ID (user can only see their own, admin can see all)
  ...
  app.patch("/api/admin/support/tickets/:id/status", isAdminAuthenticated, async (req, res) => {
    try {
      const { status } = req.body;
  ...
  app.patch("/api/admin/support/tickets/:id/assign", isAdminAuthenticated, async (req, res) => {
    try {
      const { staffId } = req.body;
      
      const ticket = await storage.assignTicket(req.params.id, staffId);
      res.json(ticket);
    } catch (error) {
  ...
  app.post("/api/reports", isAuthenticated, async (req, res) => {
    try {
      ...
      const report = await storage.createReport({
        ...
        reportedItemId: validatedData.reportedItemId,
        reportedItemType: validatedData.reportedItemType,
        reason: validatedData.reason,
        details: validatedData.details,
      });
      
      // Auto-create a support ticket for this report
      const ticket = await storage.createSupportTicket({
        userId: req.user.id,
        subject: `Report: ${validatedData.reportedItemType} - ${validatedData.reason}`,
        message: `User reported a ${validatedData.reportedItemType} (ID: ${validatedData.reportedItemId}) for reason: ${validatedData.reason}. Details: ${validatedData.details || 'N/A'}`,
      });
      
      // Link the report to the ticket
      await storage.linkReportToTicket(report.id, ticket.id);
  ...
  app.get("/api/admin/reports", isAdminAuthenticated, async (req, res) => {
    try {
      const filters: any = {};
      if (req.query.status) filters.status = req.query.status;
      if (req.query.reportedItemType) filters.reportedItemType = req.query.reportedItemType;
      
      const reports = await storage.getAllReports(filters);
      res.json(reports);
    } catch (error) {
  ...
  // Update report status and action taken (admin only)
  app.patch("/api/admin/reports/:id", isAdminAuthenticated, async (req, res) => {
    try {
      ...
      const report = await storage.updateReportStatus(req.params.id, status, actionTaken);
      
      // If closing the report, also close the linked ticket
      if (status === 'closed' && report.ticketId) {
        await storage.updateTicketStatus(report.ticketId, 'closed');
      }
